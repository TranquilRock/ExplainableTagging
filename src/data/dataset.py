...
class RelationalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        max_length = self.sentence_max_length + self.document_max_length + 8

        q_and_r_p_list = []
        for q in self.data[idx]['q']:
            if self.data[idx]['s']: # already contain SEP in tokens
                q_and_r_p = q + self.agree_token[1:] + self.data[idx]['r_p'][1:]
            else:
                q_and_r_p = q + self.disagree_token[1:] + self.data[idx]['r_p'][1:]
            padding = [PAD] * (max_length  - len(q_and_r_p))
        ...
        for r in self.data[idx]['r']:
            if self.data[idx]['s']:
                r_and_q_p = r + self.agree_token[1:] + self.data[idx]['q_p'][1:]
            else:
                r_and_q_p = r + self.disagree_token[1:] + self.data[idx]['q_p'][1:]
            padding = [PAD] * (max_length  - len(r_and_q_p))
    ...
    def _tokenize(self,
        ...
        for i, entry in enumerate(data):
            data[i]['q_p'] = tokenizer(' '.join(entry['q']), max_length=self.document_max_length, truncation=True).input_ids
            data[i]['r_p'] = tokenizer(' '.join(entry['r']), max_length=self.document_max_length, truncation=True).input_ids
            data[i]['q'] = tokenizer.batch_encode_plus(entry['q'], max_length=self.sentence_max_length, truncation=True).input_ids
            data[i]['r'] = tokenizer.batch_encode_plus(entry['r'], max_length=self.sentence_max_length, truncation=True).input_ids
