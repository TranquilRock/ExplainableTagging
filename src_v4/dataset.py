    ...
    def _formulate_item(self, x, y, s, x_labels, pid, rawx, split):
        for i in range(0, len(x), self.query_max_length):
            ...
            query = self.tokenizer.encode(query,
                                          max_length=self.query_max_length+2,
            ...
            document = self.tokenizer.encode(document,
                                             max_length=self.document_max_length+2,
                                             truncation=True, padding="max_length",
                                             is_split_into_words=True
                                             )

            relation = self.tokenizer.encode(s, is_split_into_words=True)

            input_tokens = torch.LongTensor(query + relation[1:] + document[1:])
